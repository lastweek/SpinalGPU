...
class TensorCoreBlock(config: SmConfig) extends Component {
  ...
  private val rspPayload = Reg(TensorRsp(config))
  ...
  when(io.issue.fire) {
    ...
    for (lane <- 0 until config.warpSize) {
      ...
      when(io.issue.payload.activeMask(lane)) {
        rspPayload.result(lane) :=
          (io.issue.payload.operandA(lane) * io.issue.payload.operandB(lane)).resize(config.dataWidth).asBits
      }
    }
  }
